...
module.exports = async client => {
    (await pGlob(`${process.cwd()}/commands/*/*.js`)).map(async cmdFile => {
        ...
        if (!cmd.category) return Logger.warn(`Commande non chargée: pas de category \nFichier -> ${cmdFile}`);
        ...
        if (!cmd.usage) return Logger.warn(`Commande non chargée: ajouter un usage a votre cmd \nFichier -> ${cmdFile}`);

        if (!cmd.examples) return Logger.warn(`Commande non chargée: ajouter un exemple a votre commande \nFichier -> ${cmdFile}`);

        cmd.permissions.forEach(permission => {
            if (!permissionList.includes(permission)) {
                return Logger.typo(`Evenement non-declanche: erreur de typo sur la permission ${permission} \nFichier -> ${cmdFile}`)
            }
        })
